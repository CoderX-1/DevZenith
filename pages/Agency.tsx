    ...
    <section ref={containerRef} className="h-screen bg-[#0a0a0a] overflow-hidden">
      <div ref={horizontalRef} className="flex items-center h-full px-[10vw] gap-32 will-change-transform">
        <div className="flex-shrink-0 w-[400px]">
          <h2 className="text-[10vw] font-black uppercase leading-none italic text-white/5">THE ENGINE</h2>
        </div>
        {ROADMAP_PHASES.map((phase) => (
          <div key={phase.phase} className="flex-shrink-0 w-[85vw] md:w-[600px] border border-white/10 p-16 bg-[#050505] relative group hover:border-[#FFD700]/30 transition-colors duration-700">
            <span className="text-[8vw] font-black text-white/[0.03] absolute top-8 right-8 italic">{phase.phase}</span>
            <span className="text-[10px] font-black tracking-[0.5em] text-[#FFD700] uppercase mb-12 block">PHASE_0{phase.phase}</span>
            <h3 className="text-5xl md:text-7xl font-black uppercase tracking-tighter leading-none mb-8 group-hover:text-white transition-colors">{phase.title}</h3>
            <p className="text-lg md:text-2xl font-bold uppercase tracking-tight text-white/40 leading-tight group-hover:text-white/80 transition-all">
              {phase.desc}
            </p>
          </div>
        ))}
    ...
    <div ref={containerRef} className="pt-40">
      <section className="px-6 md:px-12 mb-40">
        <div className="max-w-6xl">
          <h1 className="agency-reveal text-[10vw] md:text-[9vw] font-black uppercase tracking-tighter leading-none mb-12">
            CRAFTING <span className="text-white/20 italic">DIGITAL</span><br/>SUPREMACY.
          </h1>
          <p className="agency-reveal text-xl md:text-4xl text-white/60 leading-tight font-bold uppercase tracking-tighter max-w-4xl">
      ...
      <section className="px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 gap-20 py-48 border-t border-white/10">
        <div className="agency-reveal">
          <div className="flex items-center gap-4 mb-8">
            ...
            <span className="text-[10px] font-black tracking-[0.4em] text-[#FFD700] uppercase">PHILOSOPHY</span>
          </div>
        ...
        <div className="agency-reveal flex flex-col gap-12">
          ...
          <div className="grid grid-cols-2 gap-12 mt-8">
            {METRICS.map((m, i) => (
              <div key={i} className="flex flex-col border-t border-white/10 pt-8">
                <span className="text-5xl md:text-7xl font-black mb-2 text-white italic">{m.value}</span>
                <span className="text-[10px] font-black tracking-[0.3em] text-[#FFD700] uppercase">{m.label}</span>
              </div>
            ))}
          </div>
        </div>
